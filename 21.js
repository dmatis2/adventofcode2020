...
fs.readFile("input.txt", "utf8", (err, data) => {
    ...
    for (let allergen of allergens) {
      if (!map.has(allergen)) {
        ...
        if(tmp.length === 1) {
          ...
          map.set(allergen, ingredients.filter(i => tmp.includes(i)))
        }
      }
    }
  });
  ...
  let doneMap = new Map();
  while(map.size > 0) {
    const unique = Array.from(map.keys()).find(key => map.get(key).length === 1)
    const ingredient = map.get(unique)[0]
    doneMap.set(unique, ingredient);
    map.delete(unique)
    for(let key of map.keys()) {
      map.set(key, map.get(key).filter(x => x !== ingredient))
    }
  }
  ...
  console.log(part1);
  console.log(Array.from(doneMap.entries()).sort((a,b) => a[0].localeCompare(b[0])).map(a => a[1]).join(','));
});
