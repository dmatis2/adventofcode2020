...
fs.readFile('input.txt', 'utf8', (err, data) => {
  ...
  rules.forEach(rule => {
    ...
    rulesMap.set(type.replace(' bags ', ''), rest.trim().replace(/\d /g, '').replace(/ bag$| bags$/g,'').replace(/ bag,| bags,/g, ',').split(', '))
    }
  })
  ...
  while(queue.length !== 0) {
    ...
    for(let [rule, values] of rulesMap) {
      if(values.includes(bag) && !queue.includes(rule) && !total.includes(rule)) {
        queue.push(rule);
        total.push(rule);
      }
    }
  ...
  rules.forEach(rule => {
    const [type, rest] = rule.replace('.','').split('contain')
    rulesMap.set(type.replace(' bags ', ''), rest.trim().replace(/ bag$| bags$/g,'').replace(/ bag,| bags,/g, ',').split(', '))
  })
