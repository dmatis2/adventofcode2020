...
fs.readFile('input.txt', 'utf8', (err, data) => {
  ...
  const lines = data.split('\n\n').map(l => l.replace(/\n/g, ' ')).forEach(l => {
    const x = l.split(' ').map(x => x.split(':')).map(x => x[0]).sort()
    if ((JSON.stringify(x) === JSON.stringify(valid)) || (JSON.stringify(x) === JSON.stringify(valid2))) {
      count++;
    }
  })
  console.log(count)
  ...
  const lines2 = data.split('\n\n').map(l => l.replace(/\n/g, ' ')).forEach(l => {
    const x = l.split(' ').map(x => x.split(':')).map(x => x[0]).sort()
    if ((JSON.stringify(x) === JSON.stringify(valid)) || (JSON.stringify(x) === JSON.stringify(valid2))) {
      let obj = Object.fromEntries(l.split(' ').map(x => x.split(':')).sort())
      for (let i of valid2) {
        if (i in obj) {
          if (i === "byr") {
          ...
          if (i === "ecl") {
            obj = {...obj, [i]: ["amb","blu","brn","gry","grn","hzl","oth"].includes(obj[i])}
          }
          if (i === "pid") {
